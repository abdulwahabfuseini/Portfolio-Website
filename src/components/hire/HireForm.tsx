...
const HireForm = () => {
  ...
  const handleSubmit = async (values: FormData) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        toast.error(result.message || `Submission failed: ${response.statusText}`);
  ...
  return (
    <div className="max-w-lg mx-auto pb-24 lg:pb-0">
      ...
      >
        <Form.Item
          label={<span className="text-base font-semibold text-white">Full Name</span>}
          name="fullName"
          rules={[
        ...
        <Form.Item
          name="phoneNumber"
          label={<span className="text-base font-semibold text-white">Phone Number</span>}
          rules={[
            { required: true, message: "Please Enter Phone Number" },
            {
              ...
              validator: async (_, value: string | undefined) => {
                ...
                if (!isPossiblePhoneNumber(value)) {
                   return Promise.reject(new Error('This phone number seems too short or too long'));
                }
        ...
        <Form.Item
          label={<span className="text-base font-semibold text-white">Email Address</span>}
          name="email"
        ...
        <Form.Item
          label={<span className="text-base font-semibold text-white">Company Name (Optional)</span>}
          name="companyName"
          rules={[{ max: 100, message: "Company name cannot exceed 100 characters" }]}
        >
          <Input
            type="text"
        ...
        <Form.Item
          label={<span className="text-base font-semibold text-white">Company Address (Optional)</span>}
          name="companyAddress"
          rules={[{ max: 150, message: "Company address cannot exceed 150 characters" }]}
        >
          <Input
            type="text"
        ...
        <Form.Item
          label={<span className="text-base font-semibold text-white">Your Budget</span>}
          name="budget"
        ...
        >
          <Select
            ...
            options={[
              { value: "GH₵ 2,000.00 - GH₵ 5,000.00", label: "GH₵ 2,000.00 - GH₵ 5,000.00" },
              { value: "GH₵ 5,000.00 - GH₵ 10,000.00", label: "GH₵ 5,000.00 - GH₵ 10,000.00" },
              { value: "GH₵ 10,000.00 And Above", label: "GH₵ 10,000.00 And Above" },
            ]}
          />
        </Form.Item>
        <Form.Item
          name="detail"
          label={<span className="text-base font-semibold text-white">Project Details</span>}
        ...
        <Form.Item className="mt-4">
          <Button
            ...
            loading={loading}
            disabled={loading}
            className="text-lg text-white border-none green-background rounded h-12 w-full font-semibold hover:shadow-md disabled:opacity-70 disabled:cursor-not-allowed"
          >
